...
function hexApothem(R) { return R * Math.cos(Math.PI / 6); }
function axialDistance(a, b) {
    const aq = a.q, ar = a.r, as = -aq - ar;
    const bq = b.q, br = b.r, bs = -bq - br;
    return Math.max(Math.abs(aq - bq), Math.abs(ar - br), Math.abs(as - bs));
}
function axialToWorld(a, R_outer) {
    if (ORIENT === 'pointy') {
        // pointy-top layout
...
function generateTerrainMapRect(width, height, seed = 42) {
    ...
    const claimedByBiome = new Map(); // biome key -> count
    const targetByBiome = new Map(); // biome key -> target tiles
    ...
    for (const cfg of biomeConfigs) {
        ...
        while (placed < seedCount && idx < pool.length) {
            ...
            for (const n of axialNeighbors(c))
                if (assigned.get(key(n)) && assigned.get(key(n)) === assigned.get(k))
                    sameAdj++;
    ...
    const coordSet = new Set(coords.map(c => key(c)));
    ...
    while (unfinished() && iterations < maxIterations) {
        ...
        for (const cfg of order) {
            ...
            for (const n of neigh) {
                if (claimedByBiome.get(cfg.key) >= target)
                    break;
                const nk = key(n);
                if (!coordSet.has(nk))
                    continue; // outside rectangle
                if (assigned.has(nk))
            ...
            if (iterations % 25 === 0) {
                for (let i = frontier.length - 1; i >= 0; i--) {
                    ...
                    for (const n of axialNeighbors(c)) {
                        const nk = key(n);
                        if (coordSet.has(nk) && !assigned.has(nk)) {
                            open = true;
                            break;
                        }
                    }
...
function assignResources(tiles) { for (const t of tiles)
    t.resource = null; }
function keyOf(a) { return `${a.q},${a.r}`; }
// Visibility with terrain attenuation: forests/jungles reduce range more; mountains block.
function computeVisibleSet(tiles, center, baseRange) {
    const byKey = new Map();
    for (const t of tiles)
        byKey.set(keyOf(t), t);
    const bestRemaining = new Map();
    const q = [{ pos: center, rem: baseRange }];
    ...
    while (head < q.length) {
        ...
        for (const n of axialNeighbors(pos)) {
            ...
            if (!nTile)
                continue;
            if (nTile.type === 'mountain')
                continue; // cannot see through
...
function adjacentMountains(tilesByKey, a) {
    let count = 0;
...
function heightFor(_t) { return 0.4; }
function ResourceIcon({ t, size }) {
    ...
    return (_jsx(Text, { position: [0, heightFor(t) + 2.2, 0], fontSize: 1.2, color: "#374151", anchorX: "center", anchorY: "middle", children: label }));
}
function TreeCluster({ size, seed = 1 }) {
    ...
    return (_jsx("group", { children: trees.map((t, i) => (_jsxs("group", { position: [t.x, 0, t.z], children: [_jsxs("mesh", { position: [0, 0.3 * t.s, 0], castShadow: true, children: [_jsx("cylinderGeometry", { args: [size * 0.08 * t.s, size * 0.1 * t.s, 0.5 * t.s, 6] }), _jsx("meshStandardMaterial", { color: "#8b5a3c", roughness: 0.9, metalness: 0.0 })] }), _jsxs("mesh", { position: [0, 0.7 * t.h, 0], castShadow: true, children: [_jsx("coneGeometry", { args: [size * 0.35 * t.s, t.h, 7] }), _jsx("meshStandardMaterial", { color: "#4aa05c", roughness: 0.7, metalness: 0.0 })] })] }, i))) }));
}
function RockScatter({ size, seed = 1, count = 3 }) {
    const rng = useMemo(() => seededRand(seed), [seed]);
    const rocks = useMemo(() => {
        const arr = [];
        for (let i = 0; i < count; i++) {
    ...
    return (_jsx("group", { children: rocks.map((r, i) => (_jsxs("mesh", { position: [r.x, 0.4 * r.s, r.z], castShadow: true, children: [_jsx("icosahedronGeometry", { args: [size * 0.18 * r.s, 0] }), _jsx("meshStandardMaterial", { color: "#bfbcb6", roughness: 0.9, metalness: 0.05 })] }, i))) }));
}
function MountainDeco({ size, seed = 1 }) {
    const rng = useMemo(() => seededRand(seed), [seed]);
    const peakScale = 0.6 + rng() * 0.5;
    return (_jsxs("group", { children: [_jsxs("mesh", { position: [0, 1.2 * peakScale, 0], castShadow: true, children: [_jsx("coneGeometry", { args: [size * (0.55), 2.4 * peakScale, 6] }), _jsx("meshStandardMaterial", { color: "#9aa3a7", roughness: 0.85, metalness: 0.03 })] }), _jsxs("mesh", { position: [-size * 0.35, 0.7, size * 0.2], castShadow: true, children: [_jsx("coneGeometry", { args: [size * 0.3, 1.2, 6] }), _jsx("meshStandardMaterial", { color: "#8f989c", roughness: 0.85, metalness: 0.03 })] }), _jsxs("mesh", { position: [size * 0.4, 0.6, -size * 0.25], castShadow: true, children: [_jsx("coneGeometry", { args: [size * 0.28, 1.0, 6] }), _jsx("meshStandardMaterial", { color: "#8f989c", roughness: 0.85, metalness: 0.03 })] })] }));
}
function LakeDeco({ size }) {
    return (_jsx("group", { children: _jsxs("mesh", { rotation: [-Math.PI / 2, 0, 0], position: [0, 0.08, 0], receiveShadow: true, children: [_jsx("circleGeometry", { args: [hexApothem(size) * 0.98, 6] }), _jsx("meshStandardMaterial", { color: "#82d7ff", transparent: true, opacity: 0.9, roughness: 0.3, metalness: 0.1 })] }) }));
}
function WaterRipple({ radius }) {
    ...
    return (_jsxs("mesh", { ref: ref, rotation: [-Math.PI / 2, 0, 0], position: [0, heightFor({ q: 0, r: 0, type: 'water', char: 'L', resource: null }) + 0.06, 0], children: [_jsx("ringGeometry", { args: [radius * 0.5, radius * 0.92, 32] }), _jsx("meshBasicMaterial", { color: "#bcecff", transparent: true, opacity: 0.7 })] }));
}
// Bridge the R3F scene to the global window for reference systems that expect window.threeScene
function SceneBridge({ onReady, outerRadius }) {
    ...
    React.useEffect(() => {
        ...
        window.HEX_OUTER_RADIUS = outerRadius;
        window.HEX_APOTHEM = ap;
        window.HEX_RADIUS_3D = ap; // many ref scripts expect this name
        // Try to initialize reference systems if helper exposed an initializer
        const init = window.initialize3DSystems;
        try {
            if (typeof init === 'function')
                init();
        }
        catch { }
        const hasMountain = typeof window.addMountainToTile === 'function';
        const hasTree = typeof window.addTreeToTile === 'function';
        const hasWater = typeof window.addWaterToTile === 'function';
        const hasHills = typeof window.addHillsToTile === 'function';
        const hasDesert = typeof window.addDesertToTile === 'function';
        onReady?.({ mountain: hasMountain, tree: hasTree, water: hasWater, hills: hasHills, desert: hasDesert });
        return () => {
            if (window.threeScene === scene) {
                window.threeScene = undefined;
            }
        };
    }, [scene, onReady]);
    return null;
...
function RefMountain({ tileChar, tileX, tileY, actualHeight }) {
    ...
    React.useEffect(() => {
        ...
        const addMountain = window.addMountainToTile;
...
function RefTree({ tileChar, tileX, tileY, actualHeight, hexSize }) {
    const groupRef = React.useRef(null);
    const exposeGlobals = React.useCallback(() => {
        window.THREE = window.THREE || THREE;
    }, []);
    React.useEffect(() => {
        ...
        window.HEX_OUTER_RADIUS = hexSize;
        window.HEX_APOTHEM = ap;
        window.HEX_RADIUS_3D = ap;
        const grp = groupRef.current;
        if (grp) {
            while (grp.children.length > 0)
                grp.remove(grp.children[0]);
        }
        const addTree = window.addTreeToTile;
...
function RefWater({ tileChar, tileX, tileY, actualHeight, hexSize, }) {
    const groupRef = React.useRef(null);
    const exposeGlobals = React.useCallback(() => {
        window.THREE = window.THREE || THREE;
    }, []);
    React.useEffect(() => {
        ...
        window.HEX_APOTHEM = ap;
        window.HEX_RADIUS_3D = ap;
        window.HEX_HEIGHT_3D = actualHeight;
        const grp = groupRef.current;
        if (grp) {
            while (grp.children.length > 0)
                grp.remove(grp.children[0]);
        }
        const addWater = window.addWaterToTile;
...
function RefHills({ tileChar, tileX, tileY, actualHeight, hexSize, }) {
    const groupRef = React.useRef(null);
    const exposeGlobals = React.useCallback(() => {
        window.THREE = window.THREE || THREE;
    }, []);
    React.useEffect(() => {
        ...
        const addHills = window.addHillsToTile;
...
function RefDesert({ tileChar, tileX, tileY, actualHeight, hexSize, }) {
    const groupRef = React.useRef(null);
    const exposeGlobals = React.useCallback(() => {
        window.THREE = window.THREE || THREE;
    }, []);
    React.useEffect(() => {
        ...
        const addDesert = window.addDesertToTile;
...
function HexTile({ t, size, onClick, onHover }) {
    const h = heightFor(t);
    const color = tileColor(t);
    return (_jsx("group", { position: [0, h / 2, 0], children: _jsxs("mesh", { onClick: () => onClick(t), onPointerOver: (e) => { e.stopPropagation(); onHover?.(t); }, onPointerOut: (e) => { e.stopPropagation(); onHover?.(null); }, castShadow: true, receiveShadow: true, 
...
export default function SoloMissionMap3D() {
    ...
    const [hero, setHero] = useState({ id: 'hero', pos: { q: 0, r: 0 }, vision: 6, kind: 'actor' });
    const [pet, setPet] = useState({ id: 'pet', pos: { q: 4, r: -1 }, vision: 3, kind: 'pet' });
...
function MapCameraController({ bounds, gameMode }) {
    const { camera, gl } = useThree();
    const targetRef = React.useRef(new THREE.Vector3(0, 0, 0));
    const offsetRef = React.useRef(null); // camera.position - target
    ...
    React.useEffect(() => {
        function onMouseMove(e) {
            ...
            if (e.clientX < threshold)
                dxEdge = -(1 - e.clientX / threshold);
            else if (e.clientX > w - threshold)
                dxEdge = (1 - (w - e.clientX) / threshold);
            if (e.clientY < threshold)
                dyEdge = 1 - e.clientY / threshold; // move forward (upwards on screen)
            else if (e.clientY > h - threshold)
                dyEdge = -(1 - (h - e.clientY) / threshold);
        ...
        function onMouseDown(e) {
            if (!gameMode)
                return;
            if (e.button === 0)
                dragging.current = true; // left
            else if (e.button === 1 || e.button === 2)
                altDragging.current = true; // mid/right
            lastMouse.current = { x: e.clientX, y: e.clientY };
            // Suspend edge motion while dragging
            edgeRef.current.dx = 0;
            edgeRef.current.dy = 0;
        }
        function onMouseUp(e) { if (e.button === 0)
            dragging.current = false; if (e.button === 1 || e.button === 2)
            altDragging.current = false; }
        function onLeave() { if (!dragging.current && !altDragging.current) {
            edgeRef.current.dx = 0;
            edgeRef.current.dy = 0;
        } }
        function onWheel(e) {
    ...
    React.useEffect(() => {
        function onCtx(e) { if (dragging.current) {
            e.preventDefault();
            e.stopPropagation();
        } }
