...
function filesForGroups(groups, limitPerGroup) {
    const out = [];
    for (const g of groups) {
        const list = PART_VARIANTS.filter(v => v.group === g).map(v => v.file);
        if (limitPerGroup && list.length > limitPerGroup)
            out.push(...list.slice(0, limitPerGroup));
        else
            out.push(...list);
    }
    return out;
...
// Preload helpers
function preloadGLTF(file) { useGLTF.preload(`/assets/3d/${file}`); }
function preloadFBX(file) { useFBX.preload(`/assets/3d/${file}`); }
// Idle callback shim for wider browser support
const ric = (cb, opts) => {
    if (typeof window.requestIdleCallback === 'function')
        return window.requestIdleCallback(cb, opts);
    return window.setTimeout(() => cb({ didTimeout: false, timeRemaining: () => 0 }), opts?.timeout ?? 1);
};
function scheduleBatch(files, kind, sliceSize = 4, delayBetween = 120) {
    let idx = 0;
    function step() {
        ...
        idx += sliceSize;
        if (idx < files.length)
            setTimeout(() => ric(() => step()), delayBetween);
    }
    ric(() => step());
}
...
;
window.__AF_PRELOAD_INFO__ = { tier1, tier2, tier3, schedulePreloads };
