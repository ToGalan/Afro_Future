...
const SKILL_NAMES: Record<string, string[]> = {
  ...
  // Renamed the four generic "Faction Passive I–IV" placeholders to on-theme names
  // (political/military alliance flavour) so every branch reads as finished content.
  Faction:     ['Allied Doctrine', 'Resource Efficiency', 'Chain of Command', 'Peacekeeper Drone', 'United Front', 'Resource Redistribution', 'Banner of the People', 'Ceasefire Protocol', 'Faction Ascendancy', 'Standard Bearer', 'Rallying Banner', 'Faction Legend', 'Ascendant'],
...
// Short flavour line per tier skill (index 0 = core, left blank — the core description
// is built separately below). Keeps the snowflake's tooltips reading like finished GDD
// content instead of bare "+N STAT" text, and matches each name's implied theme.
const SKILL_FLAVOR: Record<string, string[]> = {
  Combat:      ['The foundation every soldier drills into muscle memory.', 'A heavier swing that punches through armor.', 'Hit-and-fade tactics learned in the ruins.', 'Relentless pressure that pins enemies down.', 'Channel excess energy into a single blow.', 'Frontline drills sharpen every strike.', 'A battle shout that steels your resolve.', 'Finishing technique honed on the front lines.', 'Chain attacks without pause or mercy.', 'Adrenaline turns pain into momentum.', 'Unshackled aggression, blow after blow.', 'A whirlwind of strikes overwhelms all comers.', 'Command presence that hardens every hit.'],
  Defense:     ['Engage personal plating with a thought.', 'Armor plates stiffen on impact.', 'Redirect incoming force instead of absorbing it.', 'Reinforce your stance against sustained assault.', 'Cybernetic plating locks into a hardened shell.', 'Stand as the line no enemy crosses.', 'Anchor point for allies under fire.', 'Layered defenses cycle faster than they fail.', 'Damage that would fell others barely registers.', 'Immovable footing no force can budge.', 'Plating forged for a siege that never ends.', 'Ground held is ground kept, always.', 'Your body becomes the last defense.'],
  Stealth:     ['Fold into shadow and static at will.', 'Footfalls trained to vanish into the noise floor.', "Light-bending fabric scatters a watcher's eye.", 'Strike first from a position no one expected.', 'Refined cloak discipline holds longer under scrutiny.', "Sensors sweep past you as if you weren't there.", 'Close the distance before the target reacts.', 'Full-spectrum concealment, even to thermal scans.', 'No footage, no signature, no trail.', 'Kill every signal in the area, then move.', 'The dark is your terrain, not your obstacle.', 'Disappear mid-fight and reappear on your terms.', 'Systems log your presence as noise, nothing more.'],
  Cybernetics: ['Baseline implants sharpen body and mind.', 'Neural implants shave milliseconds off every reaction.', 'Actuators in the limbs multiply raw force.', 'Direct feed sharpens targeting and timing alike.', 'Push every implant past rated limits.', 'Micro-blades self-sharpen between strikes.', 'A frame built for war, not comfort.', 'Reinforced skeleton shrugs off blows that would break bone.', 'More machine now than flesh, and stronger for it.', 'Redline every system for a burst of raw output.', 'Implants flood with power on command.', 'An experimental reactor few dare to install.', 'The line between augmentation and evolution disappears.'],
  Hacking:     ['Open doors no lock was built to stop.', 'Crack open enemy networks from the outside in.', 'Slip past defenses built to keep you out.', 'Turn a hostile drone against its own side.', 'Faster exploits, cleaner intrusions.', 'A pulse that fries unshielded electronics on contact.', 'Overrun a defended system node by node.', 'No firewall built has held you out for long.', 'Enemy systems answer to you now.', 'Countermeasures that bite back against intruders.', 'Buried access no admin will ever find.', "An exploit no one has patched — yet.", "You move through networks like they're your own home."],
  Healing:     ['Patch wounds fast enough to keep moving.', 'Tissue knits faster than the wounds that made it.', 'Conditioning that raises your ceiling for punishment.', 'Close a wound before it slows you down.', 'Microscopic repair swarms patch damage on the move.', 'A zone where wounds simply heal faster.', 'Recovery that never fully stops, even mid-fight.', 'Every respite restores more than it used to.', 'Vitality spreads outward to those nearby.', 'A flood of restored strength when it matters most.', 'Bring a body back from the brink.', 'The will to keep fighting when the body says stop.', 'Your presence alone keeps allies standing.'],
  'Pet Bond':  ['The first thread of trust between handler and beast.', 'Simple signals your companion follows without hesitation.', 'Send your companion ahead to scout the ground.', 'Coordinated strikes hit harder than either of you alone.', 'Layered commands for combat, not just obedience.', 'Sharper claws, stronger bite, better odds.', 'Instinct and loyalty fuse into something fiercer.', 'Implants link handler and companion as one system.', "Your bond reshapes what your pet is capable of.", 'A command that rallies every beast in earshot.', 'Unleash the wild your companion never lost.', 'Bred and trained for the front line.', "A bond deep enough to feel each other's pain."],
  Faction:     ["Standard training shared across the faction's ranks.", 'Faction logistics stretch every supply further.', 'Clear orders move faster through the ranks.', 'An automated unit that holds contested ground.', 'Coordinated faction support strengthens your position.', 'Route surplus supplies to where they matter.', "Rally the faction's full backing behind you.", 'Broker a pause that buys time to regroup.', "Your faction's influence expands with every action.", 'Carry the banner that holds the line together.', 'Allies fight harder in your presence.', 'Your name carries weight across every outpost.', "Recognized leadership within the faction's ranks."],
  Terraform:   ['Turn the first patch of ash back into soil.', 'Coax the first green shoots from dead ground.', 'Strip toxins from the earth, one plot at a time.', 'Fields recover faster with every cycle.', 'A wave of restoration ripples outward from you.', 'Barren ground gives way to living color.', "Machinery that speeds nature's own repair work.", 'Reshape land faster than the wasteland can claim it.', 'New ecosystems take root where none stood before.', 'What was lost to the wasteland, you take back.', 'Restoration spreads faster than it can be undone.', 'A living landscape rises under your hands.', 'The land itself answers to your care.'],
  Mobility:    ['Conditioning that puts spring back in your step.', 'Lighter steps cover more ground per stride.', 'Conditioning trims every wasted motion.', 'A burst of speed that closes gaps fast.', 'Read an attack early enough to step clear of it.', 'Momentum carries you further than legs alone.', 'Servo-assisted stride built for open ground.', "Cut through resistance like it isn't there.", 'Too fast to pin down, too quick to hit.', 'Movement too fast for the eye to track cleanly.', "By the time they react, you're already gone.", 'Barely touch the ground between strides.', 'Speed that borders on the impossible.'],
  Diplomacy:   ['Words chosen carefully enough to end a fight before it starts.', 'Words that steady allies when the line wavers.', 'Confidence spreads through the ranks around you.', 'Buy a pause in the fighting through words alone.', 'Read the field and turn words into advantage.', 'Lead negotiations from a position of strength.', 'Trusted to speak for more than yourself.', 'Find the compromise that ends a standoff.', 'Formal terms that bind rival factions to peace.', 'Influence that reaches beyond the battlefield.', 'Bring divided factions to the same table.', 'A ceasefire that actually holds.', 'Lasting agreement built on respect, not just leverage.'],
  Scavenging:  ['An eye trained to spot what others discard.', 'Strip usable parts before anyone else arrives.', 'Portable tools make short work of wreckage.', "A hidden stash saved for when it's needed most.", 'Take what the wasteland already gave up on.', 'Reserves that keep growing, mission after mission.', 'Leave nothing of value behind.', 'An eye for value others walk right past.', 'Recover components most scavengers would miss.', 'Every scrap has a use eventually.', 'Know where the wasteland hides its best finds.', 'Rare finds turn up more often under your watch.', "Your reserves rival a faction's own supply lines."],
};
...
export function makeTree(): SkillNode[] {
  ...
  BRANCHES.forEach(b => {
    ...
    const flavor = SKILL_FLAVOR[b.name] || [];
    const statLabel = b.stat === 'atk' ? 'ATK' : b.stat === 'def' ? 'DEF' : 'UTIL';
    // Core node (tier 1) — castable ability when the branch defines a bar/effect.
    const coreFlavor = flavor[0] ? `${flavor[0]} ` : '';
    nodes.push({
      id: `${baseId}_core`, label: names[0] || `${b.name} Core`,
      description: b.effect ? `${coreFlavor}Castable ability. +1 ${statLabel}. (1 pt)` : `${coreFlavor}${b.name} fundamentals. +1 ${statLabel}. (1 pt)`,
    ...
    for (let i = 0; i < TIER_NODES; i++) {
      ...
      const line = flavor[i + 1];
      nodes.push({
        id, label: names[i + 1] || `${b.name} ${i + 1}`,
        description: line ? `${line} +${amount} ${statLabel}. (${cost} pts)` : `+${amount} ${statLabel}. (${cost} pts)`,
