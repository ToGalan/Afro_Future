...
export default function App() {
    ...
    return (_jsx(GameViewport, { mode: "fit", children: _jsx(MainMenu, { playerName: playerName, accountLevel: accountLevel, loadout: activeLoadout ?? defaultLoadout, onCustomize: () => setPhase('creating'), heroLocked: heroLocked, view: mainView, onChangeView: setMainView }) }));
}
...
function MainMenu({ playerName, accountLevel, loadout, onCustomize, heroLocked, view, onChangeView }) {
    return (_jsxs("div", { className: "h-full w-full bg-[#0f1218] text-gray-100 grid grid-rows-[64px_1fr]", style: { gridTemplateColumns: 'minmax(260px,20%) 1fr minmax(260px,20%)' }, children: [_jsx(WebGLContextBanner, {}), _jsx(TopNav, { view: view, onChangeView: onChangeView }), _jsx(LeftPlayerPanel, { className: "row-start-2", playerName: playerName, accountLevel: accountLevel, loadout: loadout, onCustomize: onCustomize, heroLocked: heroLocked }), _jsx(CenterHub, { className: "row-start-2", loadout: loadout, view: view }), _jsx(RightStartPanel, { className: "row-start-2" })] }));
}
function TopNav({ view, onChangeView }) {
    return (_jsxs("div", { className: "col-span-3 grid grid-cols-3 items-center px-6 bg-[#141924] border-b border-white/10 h-16", children: [_jsx("div", { className: "flex items-center", children: _jsxs("button", { onClick: () => onChangeView('dashboard'), className: "flex items-center gap-2 group", title: "Home", children: [_jsx("div", { className: "w-9 h-9 rounded-lg bg-gradient-to-br from-emerald-500/30 to-sky-500/30 border border-white/10 flex items-center justify-center font-bold text-sm tracking-wide text-emerald-200 group-hover:from-emerald-500/50 group-hover:to-sky-500/50 transition", children: "AF" }), _jsx("span", { className: "text-sm font-semibold tracking-wide bg-gradient-to-r from-emerald-300 to-sky-300 bg-clip-text text-transparent hidden xl:inline-block", children: "Afro\u2011Future" })] }) }), _jsxs("div", { className: "flex items-center justify-center gap-8", children: [_jsx("button", { className: `opacity-80 hover:opacity-100 transition ${view === 'skills' ? 'text-emerald-400' : ''}`, onClick: () => onChangeView('skills'), children: "Skills" }), _jsx("button", { className: "opacity-80 hover:opacity-100", children: "Progress" }), _jsx("button", { className: "opacity-80 hover:opacity-100", children: "Store" }), _jsx("button", { className: "opacity-80 hover:opacity-100", children: "Help" })] }), _jsxs("div", { className: "ml-auto flex items-center justify-end gap-4", children: [_jsxs(Chip, { children: ["1,458 ", _jsx("span", { className: "opacity-70", children: "shards" })] }), _jsx(IconButton, { label: "Notifications", children: "\uD83D\uDD14" }), _jsx(IconButton, { label: "Mail", children: "\u2709\uFE0F" }), _jsx("img", { className: "w-9 h-9 rounded object-cover border border-white/10", src: "https://images.unsplash.com/photo-1546527868-ccb7ee7dfa6a?q=80&w=200&auto=format&fit=crop", alt: "avatar" }), _jsx(Button, { size: "sm", children: "Profile" })] })] }));
}
function LeftPlayerPanel({ className = '', playerName, accountLevel, loadout, onCustomize, heroLocked }) {
    ...
    const baseHP = 100;
    const hpPerLevel = 12;
    const hpPerDefense = 4;
    const baseEP = 60;
    const epPerLevel = 6;
    const epPerUtility = 3;
    const HP = baseHP + (skillLevel - 1) * hpPerLevel + defense * hpPerDefense;
    const EP = baseEP + (skillLevel - 1) * epPerLevel + utility * epPerUtility;
    function statDots(v) { const capped = Math.min(10, Math.round(v / 2)); return Array.from({ length: 5 }).map((_, i) => i < Math.ceil(capped / 2) ? '●' : '○').join(''); }
    return (_jsxs("aside", { className: `col-start-1 h-full ${className} bg-[#0f1218] border-r border-black/40 shadow-inner flex flex-col`, children: [_jsxs("div", { className: "px-4 py-4 border-b border-white/10 flex flex-col items-center", children: [_jsxs("div", { className: "w-[240px] h-[260px] rounded-3xl border border-white/10 bg-[#12171f] overflow-hidden relative flex flex-col items-stretch", children: [_jsxs("div", { className: "absolute inset-0", children: [_jsx(AvatarScene, { parts: loadout.threeConfig?.parts, colors: loadout.threeConfig?.colors, debugTint: false, animPaused: false, animSpeed: 1, rotateSpeed: 0.08, disableControls: true, cameraPosition: [1.65, 1.4, 2.15], cameraFov: 33, target: [0, 1.05, 0], modelOffset: [0, -0.35, 0], autoFrame: true, frameMargin: 0.14 }), _jsx("div", { className: "absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-[#0b0e13] to-transparent" })] }), _jsx("img", { className: "absolute top-2 right-2 w-16 h-16 object-cover rounded-xl border border-white/10 shadow-lg opacity-80", src: loadout.portraitUrl, alt: "portrait" }), _jsxs("div", { className: "relative z-10 px-3 pb-3 pt-2 flex flex-col items-center text-center mt-auto", children: [_jsxs("div", { className: "flex items-center gap-2 mb-1", children: [_jsx("img", { src: FactionIcon[loadout.faction], alt: loadout.faction, className: "w-5 h-5 drop-shadow" }), _jsx("span", { className: "font-semibold text-white text-sm leading-tight tracking-wide", children: loadout.name })] }), _jsxs("div", { className: "text-[10px] text-gray-300 flex items-center gap-2", children: [_jsxs("span", { className: "px-2 py-0.5 rounded-full bg-white/5 border border-white/10 tracking-wide", children: ["Lv ", loadout.level] }), _jsx("span", { children: loadout.archetype === 'MALE' ? 'Male' : 'Female' })] })] }), _jsxs("span", { className: "absolute top-2 left-2 px-2 py-1 rounded-lg bg-black/50 border border-white/10 text-[10px] tracking-wide flex items-center gap-1", children: [_jsx("img", { src: FactionIcon[loadout.faction], alt: "faction", className: "w-3.5 h-3.5" }), loadout.faction] })] }), _jsxs("div", { className: "mt-4 grid grid-cols-3 gap-3 w-full", children: [_jsxs("div", { className: "rounded-2xl border border-white/10 bg-white/5 p-3 h-24 flex flex-col items-center justify-center text-[11px] text-center", children: [_jsx("div", { className: "opacity-70 mb-1", children: "Attack" }), _jsx("div", { className: "font-semibold tracking-wider", children: statDots(attack) }), _jsx("div", { className: "text-[9px] opacity-60 mt-1", children: attack })] }), _jsxs("div", { className: "rounded-2xl border border-white/10 bg-white/5 p-3 h-24 flex flex-col items-center justify-center text-[11px] text-center", children: [_jsx("div", { className: "opacity-70 mb-1", children: "Defense" }), _jsx("div", { className: "font-semibold tracking-wider", children: statDots(defense) }), _jsx("div", { className: "text-[9px] opacity-60 mt-1", children: defense })] }), _jsxs("div", { className: "rounded-2xl border border-white/10 bg-white/5 p-3 h-24 flex flex-col items-center justify-center text-[11px] text-center", children: [_jsx("div", { className: "opacity-70 mb-1", children: "Utility" }), _jsx("div", { className: "font-semibold tracking-wider", children: statDots(utility) }), _jsx("div", { className: "text-[9px] opacity-60 mt-1", children: utility })] })] }), _jsxs("div", { className: "mt-4 w-full grid grid-cols-2 gap-3", children: [_jsxs("div", { className: "rounded-2xl border border-white/10 bg-gradient-to-br from-emerald-500/10 to-sky-500/10 p-3 flex flex-col text-[10px]", children: [_jsxs("div", { className: "flex justify-between", children: [_jsx("span", { className: "opacity-70", children: "HP" }), _jsx("span", { className: "font-semibold", children: HP })] }), _jsxs("div", { className: "flex justify-between mt-1", children: [_jsx("span", { className: "opacity-70", children: "EP" }), _jsx("span", { className: "font-semibold", children: EP })] }), _jsxs("div", { className: "flex justify-between mt-1", children: [_jsx("span", { className: "opacity-70", children: "Skill Lv" }), _jsx("span", { className: "font-semibold", children: skillLevel })] })] }), _jsxs("div", { className: "rounded-2xl border border-white/10 bg-white/5 p-3 flex flex-col text-[10px]", children: [_jsxs("div", { className: "flex justify-between", children: [_jsx("span", { className: "opacity-70", children: "Tokens Spent" }), _jsx("span", { className: "font-semibold", children: skillSpent })] }), _jsxs("div", { className: "flex justify-between mt-1", children: [_jsx("span", { className: "opacity-70", children: "Tokens Left" }), _jsx("span", { className: "font-semibold text-emerald-300", children: pointsLeft })] }), _jsxs("div", { className: "flex justify-between mt-1", children: [_jsx("span", { className: "opacity-70", children: "Unlocked" }), _jsx("span", { className: "font-semibold", children: skillUnlocked.length })] })] })] }), _jsx("div", { className: "mt-4 w-full flex items-center justify-center", children: _jsx(Button, { className: "w-[160px]", onClick: onCustomize, children: "Customize" }) }), _jsxs("div", { className: "mt-4 w-full rounded-2xl border border-white/10 bg-white/5 p-3 text-[10px] flex flex-col gap-1", children: [_jsxs("div", { className: "flex justify-between", children: [_jsx("span", { className: "opacity-70", children: "Primary Path" }), _jsx("span", { className: "font-semibold", children: primaryBranch || '—' })] }), _jsxs("div", { className: "flex justify-between", children: [_jsx("span", { className: "opacity-70", children: "Primary Type" }), _jsx("span", { className: "font-semibold", children: primaryType || '—' })] }), _jsx("div", { className: "mt-1 flex flex-wrap gap-1", children: traitTags.length ? traitTags.map(t => _jsx("span", { className: "px-2 py-0.5 rounded-full bg-white/10 border border-white/10 text-[9px] tracking-wide", children: t }, t)) : (_jsx("span", { className: "opacity-50", children: "Earn traits by investing tokens" })) })] })] }), _jsxs("div", { className: "px-4 py-6 flex flex-col items-center gap-4", children: [_jsxs("div", { className: "w-[240px] h-[240px] rounded-3xl border border-white/10 bg-white/5 overflow-hidden relative flex flex-col items-center justify-center p-4", children: [_jsx("div", { className: "absolute inset-0 bg-gradient-to-tr from-[#0b0e13]/80 via-transparent to-[#0b0e13]/50" }), _jsx("img", { className: "relative z-10 w-28 h-28 object-contain drop-shadow", src: PetIcon[loadout.pet.type], alt: "pet" }), _jsxs("div", { className: "relative z-10 mt-3 flex flex-col items-center text-center", children: [_jsx("div", { className: "font-semibold text-white text-base leading-tight", children: loadout.pet.type === 'CYBER_DOG' ? 'Cyber-Dog' : 'Cyber-Cat' }), _jsxs("div", { className: "mt-1 text-[11px] text-gray-300 flex items-center gap-2", children: [_jsxs("span", { className: "px-2 py-0.5 rounded-full bg-white/5 border border-white/10 tracking-wide", children: ["Lv ", loadout.pet.level] }), _jsx("span", { children: loadout.pet.role })] })] })] }), _jsx("div", { className: "grid grid-cols-3 gap-3 w-full", children: [
                            { label: 'Attack', value: '●●○○○' },
                            { label: 'Support', value: '●●●○○' },
                            { label: 'Agility', value: '●●●●○' },
...
function VariantCard({ v, active, onSelect, eager }) {
    ...
    const sharedApi = (() => { try {
        return useSharedPreview();
    }
    catch {
        return null;
    } })();
    return (_jsx("button", { onClick: onSelect, title: v.label, onMouseEnter: () => {
            setHover(true);
            if (shared && sharedApi && ref.current)
                sharedApi.setCardPreview(v.file, ref.current, active);
        }, onMouseLeave: () => {
            setHover(false);
            if (shared && sharedApi)
                sharedApi.clearIfTransient(v.file);
        }, className: `w-24 h-24 rounded-xl border transition flex flex-col items-center justify-center gap-1 px-1 text-[11px]
        ${active ? 'bg-emerald-600/30 border-emerald-500/60 text-emerald-200 shadow-inner' : 'bg-white/5 border-white/10 hover:bg-white/10 text-gray-300'}`, children: _jsxs("div", { ref: ref, "data-variant-card": true, "data-file": v.file, className: "relative w-full h-full flex items-center justify-center", children: [!shared && show3D ? _jsx(VariantPreview, { file: v.file }) : (!shared && (_jsx("div", { className: "w-10 h-10 rounded-lg bg-gradient-to-br from-white/10 to-white/5 flex items-center justify-center text-[10px] opacity-60", children: "GLB" }))), shared && (_jsx("div", { className: "w-10 h-10 rounded-lg bg-gradient-to-br from-white/10 to-white/5 flex items-center justify-center text-[10px] opacity-50", children: "GLB" })), _jsxs("div", { className: "absolute bottom-0 left-0 right-0 text-[9px] leading-tight px-1 py-0.5 bg-black/40 backdrop-blur-sm", children: [_jsx("span", { className: "truncate block max-w-full", children: v.label }), active && _jsx("span", { className: "uppercase tracking-wide text-emerald-300", children: "Selected" })] })] }) }));
}
