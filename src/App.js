...
export default function App() {
    ...
    useEffect(() => {
        ...
        window.addEventListener('af:openInviteDialog', onOpen);
        return () => window.removeEventListener('af:openInviteDialog', onOpen);
    }, []);
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (synced) {
                    const { ['afrofuture.profile']: prof, ['afrofuture.loadout']: loadout, ['afrofuture.skills']: skills } = synced;
                    ...
                    if (loadout && loadout.id && !activeLoadout) {
                        const normalized = {
                            ...loadout,
                            portraitUrl: getCharacterPortrait(loadout.faction, loadout.archetype),
                        };
    ...
    return (mainView === 'mission' ? (_jsx(MissionScreen, { onExit: () => setMainView('dashboard') })) : (_jsxs(GameViewport, { mode: "fit", children: [_jsx(MainMenu, { playerName: playerName, accountLevel: accountLevel, loadout: activeLoadout ?? defaultLoadout, onCustomize: () => setPhase('creating'), heroLocked: heroLocked, view: mainView, onChangeView: setMainView, profile: profile, onSignOut: handleSignOut }), inviteOpen && (_jsx(InviteDialog, { onClose: () => setInviteOpen(false) }))] })));
}
function MissionScreen({ onExit }) {
    ...
    const skillIcon = React.useCallback((id) => {
        if (id.includes('combat'))
            return '⚔️';
        if (id.includes('support'))
            return '✚';
        if (id.includes('pet') || id.includes('bond'))
            return '🐾';
        ...
        if (id.includes('mobility'))
            return '🦶';
        if (id.includes('leadership'))
            return '🗣️';
        if (id.includes('terraform'))
            return '🌍';
        if (id.includes('technologist') || id.includes('techno'))
            return '🧪';
        if (id.includes('merchant') || id.includes('trade'))
            return '💱';
        if (id.includes('looting'))
            return '📦';
        return '⬢';
    }, []);
    // Local runtime ability state (cooldowns & disabled flags)
    const [runtimeAbilities, setRuntimeAbilities] = React.useState({});
    ...
    const loadout = activeLoadoutRaw || { name: 'Hero', faction: 'PAA', level, portraitUrl: undefined, pet: { type: 'CYBER_DOG', level: 1, role: 'SCOUT' } };
    ...
    const onActivateAbility = React.useCallback((id) => {
        setRuntimeAbilities(prev => {
            const cur = prev[id];
            if (cur && cur.cooldown > 0)
                return prev; // still cooling
            return { ...prev, [id]: { cooldown: (cur?.max) || BASE_COOLDOWN, max: (cur?.max) || BASE_COOLDOWN } };
        });
        // EP cost heuristic
        setHeroVitals(v => ({ ...v, ep: Math.max(0, v.ep - 10) }));
    }, []);
    return (_jsxs("div", { className: "fixed inset-0 bg-[#06080c] text-gray-100", children: [_jsxs("div", { className: "absolute inset-0", children: [_jsx(SoloMissionMap3D, {}), _jsx(PetPanel, {}), import.meta.env.DEV && _jsx(React.Suspense, { fallback: null, children: _jsx("div", { children: _jsx(DatabaseTestPanel, {}) }) })] }), _jsx(GameHUD, { team: loadout.faction, clock: '00:00', score: { radiant: 0, dire: 0 }, hero: {
...
function InviteDialog({ onClose }) {
    ...
    async function submit(e) {
        ...
        try {
            ...
            if (json.ok) {
                setResult({ ok: true, code: json.code, emailed: json.emailed });
            }
            else
                setResult({ ok: false, error: json.error || 'invite_failed' });
        }
        catch (e) {
            setResult({ ok: false, error: e?.message || 'net_failed' });
        }
        finally {
            setSubmitting(false);
        }
    }
    return (_jsxs("div", { className: "fixed inset-0 z-[200] flex items-center justify-center", children: [_jsx("div", { className: "absolute inset-0 bg-black/70 backdrop-blur-sm", onClick: onClose }), _jsxs("form", { onSubmit: submit, className: "relative w-full max-w-md mx-auto rounded-2xl border border-white/10 bg-[#1b222c]/95 p-6 flex flex-col gap-4 shadow-2xl", children: [_jsxs("div", { className: "flex items-start justify-between", children: [_jsxs("div", { children: [_jsx("h2", { className: "text-lg font-semibold tracking-wide", children: "Invite a Friend" }), _jsx("p", { className: "text-[11px] opacity-70 mt-1 leading-relaxed", children: "Send an invitation link. They'll receive a unique code and future rewards logic can hook into this." })] }), _jsx("button", { type: "button", onClick: onClose, className: "w-8 h-8 -m-2 rounded-lg flex items-center justify-center text-xs bg-white/5 hover:bg-white/10 border border-white/10", children: "\u2715" })] }), _jsxs("label", { className: "flex flex-col gap-1 text-xs", children: [_jsx("span", { className: "opacity-70", children: "Friend's Email" }), _jsx("input", { value: email, onChange: e => setEmail(e.target.value), type: "email", required: true, placeholder: "friend@example.com", className: "h-10 px-3 rounded-lg bg-black/40 border border-white/10 focus:outline-none focus:border-emerald-400/60 text-sm" })] }), _jsxs("label", { className: "flex flex-col gap-1 text-xs", children: [_jsx("span", { className: "opacity-70", children: "Message (optional)" }), _jsx("textarea", { value: message, onChange: e => setMessage(e.target.value), rows: 3, maxLength: 500, className: "resize-none px-3 py-2 rounded-lg bg-black/40 border border-white/10 focus:outline-none focus:border-emerald-400/60 text-[13px]" })] }), _jsxs("div", { className: "flex items-center justify-between text-[11px]", children: [_jsx("span", { className: "opacity-60", children: "The invite creates a share URL + attempts email send." }), _jsx("span", { className: "font-mono " + (valid ? 'text-emerald-300' : 'text-rose-300'), children: valid ? 'valid' : 'invalid' })] }), result && (_jsx("div", { className: "text-[11px] rounded-lg border px-3 py-2 " + (result.ok ? 'border-emerald-500/30 bg-emerald-900/20 text-emerald-200' : 'border-rose-500/30 bg-rose-900/20 text-rose-200'), children: result.ok ? (_jsxs("div", { children: ["Invite created. Code: ", _jsx("span", { className: "font-mono", children: result.code }), ' ', "\u00B7 ", result.emailed ? 'Email sent.' : 'Email not sent (transport not configured).'] })) : (_jsxs("div", { children: ["Invite failed: ", result.error] })) })), _jsxs("div", { className: "flex items-center gap-3 pt-1", children: [_jsxs("button", { type: "submit", disabled: !valid || submitting, className: "h-10 px-5 rounded-lg bg-gradient-to-r from-emerald-600 to-sky-600 disabled:opacity-40 text-sm font-medium tracking-wide border border-white/10 hover:from-emerald-500 hover:to-sky-500 flex items-center gap-2", children: [submitting && _jsx("span", { className: "w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" }), _jsx("span", { children: submitting ? 'Sending…' : 'Send Invite' })] }), _jsx("button", { type: "button", onClick: onClose, className: "h-10 px-4 rounded-lg bg-white/10 hover:bg-white/15 border border-white/10 text-sm", children: "Cancel" })] })] })] }));
}
        ...
        try {
            ...
            await initGoogleIdentity(cid, (resp) => {
                try {
                    ...
                    console.warn('[auth] credential handling failed', e);
                }
            });
...
function MainMenu({ playerName, accountLevel, loadout, onCustomize, heroLocked, view, onChangeView, profile, onSignOut }) {
    ...
    return (_jsxs("div", { className: "h-full w-full bg-[#0f1218] text-gray-100", children: [_jsx(TopNav, { view: view, onChangeView: onChangeView, profile: profile, onSignOut: onSignOut }), _jsx("div", { className: "h-[calc(100%-64px)]", children: _jsxs("div", { className: "h-full grid grid-rows-[1fr]", style: { gridTemplateColumns: 'minmax(260px,20%) 1fr minmax(260px,20%)' }, children: [_jsx(LeftPlayerPanel, { className: "row-start-1", playerName: playerName, accountLevel: accountLevel, loadout: loadout, heroLocked: heroLocked }), _jsx(CenterHub, { className: "row-start-1", loadout: loadout, view: view }), _jsx(RightPlayerPanel, { className: "row-start-1", loadout: loadout, onCustomize: onCustomize, onPlay: () => onChangeView('mission') })] }) })] }));
}
...
function LeftPlayerPanel({ className = '', playerName, accountLevel, loadout, heroLocked }) {
    ...
    React.useEffect(() => {
        if (!shareOpen)
            return;
        function onDown(e) { if (shareRef.current && !shareRef.current.contains(e.target))
            setShareOpen(false); }
    ...
    function characterShareUrl() {
        ...
        if (loadout.portraitUrl) {
            try {
                params.set('portrait', new URL(loadout.portraitUrl, location.origin).toString());
            }
            catch { }
        }
        // Include hero name for /og/card endpoint usage by social scrapers
        if (loadout.name)
            params.set('name', encodeURIComponent(loadout.name));
        return `${base}/?share=${params.toString()}`;
    }
    function copyShare() {
        try {
            navigator.clipboard.writeText(characterShareUrl());
        }
        catch { }
    }
    async function doShare(target) {
        ...
        if (target === 'system' && navigator.share) {
            try {
                ...
                await navigator.share({ title: 'Afro-Future Hero', text: baseText, url, files });
                setShareOpen(false);
                return;
            }
            catch { }
...
function RightPlayerPanel({ className = '', loadout, onCustomize, onPlay }) {
...
function CenterHub({ className = '', loadout, view }) {
    ...
    if (view === 'mission') {
        return (_jsx("main", { className: `col-start-2 px-6 py-5 min-h-0 flex flex-col ${className}`, children: _jsx("div", { className: "flex-1 rounded-3xl border border-white/10 bg-[#12171f] overflow-hidden", children: _jsx("div", { className: "w-full h-full", children: _jsx(SoloMissionMap3D, {}) }) }) }));
    }
    if (view === 'help') {
        return _jsx(HelpView, { className: className });
    }
    if (view === 'settings') {
...
function SettingsView({ className = '' }) {
    const version = (typeof window !== 'undefined' && window.__AF_ENV?.buildHash)
        ? window.__AF_ENV.buildHash
        : APP_VERSION_FALLBACK;
    return (_jsx("main", { className: `col-start-2 px-6 py-5 min-h-0 flex flex-col ${className}`, children: _jsxs("div", { className: "flex-1 rounded-3xl border border-white/10 bg-[#12171f] overflow-hidden p-6 space-y-8", children: [_jsxs("section", { children: [_jsx("h2", { className: "text-xl font-semibold mb-2", children: "Settings" }), _jsx("p", { className: "text-sm opacity-80 leading-relaxed", children: "Configure your experience. (Prototype \u2013 values may not persist yet.)" })] }), _jsxs("section", { className: "space-y-4", children: [_jsxs("div", { className: "rounded-xl border border-white/10 bg-white/5 p-4 flex flex-col gap-3", children: [_jsx("div", { className: "text-sm font-semibold tracking-wide", children: "Audio" }), _jsxs("div", { className: "flex items-center justify-between text-xs", children: [_jsx("span", { children: "Master Volume" }), _jsx("input", { "aria-label": "Master Volume", type: "range", min: 0, max: 100, defaultValue: 80 })] }), _jsxs("div", { className: "flex items-center justify-between text-xs", children: [_jsx("span", { children: "Music" }), _jsx("input", { "aria-label": "Music Volume", type: "range", min: 0, max: 100, defaultValue: 60 })] })] }), _jsxs("div", { className: "rounded-xl border border-white/10 bg-white/5 p-4 flex flex-col gap-3", children: [_jsx("div", { className: "text-sm font-semibold tracking-wide", children: "Graphics" }), _jsxs("div", { className: "flex items-center justify-between text-xs", children: [_jsx("span", { children: "Quality" }), _jsxs("select", { className: "bg-transparent border border-white/20 rounded px-2 py-1 text-xs", children: [_jsx("option", { children: "Auto" }), _jsx("option", { children: "Low" }), _jsx("option", { children: "Medium" }), _jsx("option", { children: "High" })] })] })] }), _jsxs("div", { className: "rounded-xl border border-white/10 bg-white/5 p-4 flex flex-col gap-3", children: [_jsx("div", { className: "text-sm font-semibold tracking-wide", children: "Account" }), _jsx("div", { className: "text-xs opacity-70", children: "More account controls coming soon." })] })] }), _jsx("div", { className: "mt-8 text-[4pt] leading-none opacity-60 select-text", children: version })] }) }));
}
// --- Help View ---
function HelpView({ className = '' }) {
    const items = [
    ...
    const chunks = React.useMemo(() => {
        ...
        for (let i = 0; i < pages; i++)
            out.push(items.slice(i * 2, i * 2 + 2));
        return out;
    }, [pages, items.length]);
    const go = (d) => setPage(p => Math.max(0, Math.min(p + d, pages - 1)));
    return (_jsx("main", { className: `col-start-2 px-6 py-5 min-h-0 flex flex-col ${className}`, children: _jsxs("div", { className: "flex-1 rounded-3xl border border-white/10 bg-[#12171f] overflow-hidden p-0", children: [_jsxs("div", { className: "relative h-48 sm:h-56 md:h-64 bg-gradient-to-r from-emerald-600/30 via-sky-600/20 to-indigo-600/20 border-b border-white/10", children: [_jsx("div", { className: "absolute inset-0 bg-[radial-gradient(ellipse_at_top_left,rgba(16,185,129,0.25),transparent_60%)]" }), _jsxs("div", { className: "absolute bottom-4 left-6", children: [_jsx("h2", { className: "text-2xl md:text-3xl font-semibold", children: "How to Play" }), _jsx("div", { className: "text-xs opacity-80", children: "A quick visual guide to get you started" })] })] }), _jsxs("div", { className: "relative p-6", children: [_jsx("button", { "aria-label": "Prev", onClick: () => go(-1), disabled: page === 0, className: "absolute left-3 top-1/2 -translate-y-1/2 h-9 w-9 rounded-full bg-black/40 border border-white/10 hover:bg-black/50 disabled:opacity-40 disabled:cursor-not-allowed grid place-items-center z-10", children: "\u2039" }), _jsx("button", { "aria-label": "Next", onClick: () => go(1), disabled: page >= pages - 1, className: "absolute right-3 top-1/2 -translate-y-1/2 h-9 w-9 rounded-full bg-black/40 border border-white/10 hover:bg-black/50 disabled:opacity-40 disabled:cursor-not-allowed grid place-items-center z-10", children: "\u203A" }), _jsx("div", { className: "overflow-hidden", children: _jsx("div", { className: "flex transition-transform duration-300", style: { transform: `translateX(-${page * 100}%)` }, children: chunks.map((group, idx) => (_jsx("div", { className: "w-full shrink-0 px-1", children: _jsx("div", { className: "grid grid-cols-2 gap-4", children: group.map((s, i) => (_jsxs("div", { className: "rounded-2xl border border-white/10 bg-white/[0.04] p-5 flex flex-col gap-3", children: [_jsx("div", { className: "text-2xl select-none", children: s.icon }), _jsx("div", { className: "text-sm font-semibold tracking-wide", children: s.title }), _jsx("div", { className: "text-xs opacity-75 leading-relaxed", children: s.desc }), _jsx("div", { className: "mt-2 h-24 rounded-lg bg-gradient-to-br from-black/10 to-white/5 border border-white/10 flex items-center justify-center text-[10px] opacity-60", children: "Illustration" })] }, i))) }) }, idx))) }) }), _jsx("div", { className: "mt-4 flex items-center justify-center gap-2", children: Array.from({ length: pages }).map((_, i) => (_jsx("span", { className: `w-1.5 h-1.5 rounded-full ${i === page ? 'bg-white/80' : 'bg-white/30'}` }, i))) })] })] }) }));
}
...
function CharacterCreator({ onSave, onBack, initial, locked }) {
    ...
    const [picked, setPicked] = useState(() => {
        const parts = initial?.threeConfig?.parts;
        return (parts && typeof parts === 'object') ? { ...parts } : {};
    });
    const [colorState, setColorState] = useState(() => {
        const colors = initial?.threeConfig?.colors;
        return {
            primary: colors?.primary || '#00A37A',
            secondary: colors?.secondary || '#F5F5F5',
            skin: colors?.skin || '#c58b66',
        };
    });
