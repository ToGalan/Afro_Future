...
export default function App() {
    ...
    if (phase === 'auth')
        return _jsx(GameViewport, { children: _jsx(AuthGate, { onSignedIn: handleSignedIn }) });
    if (phase === 'boot')
        return _jsx(GameViewport, { children: _jsx(WelcomeScreen, { progress: progress, build: "v0.2.3 demo \u2022 UE5", onSignOut: handleSignOut }) });
    if (phase === 'onboard')
        return (_jsx(GameViewport, { children: _jsx(FirstTimeFlow, { faction: setupFaction, archetype: setupArchetype, pet: setupPet, onFaction: setSetupFaction, onArchetype: setSetupArchetype, onPet: setSetupPet, onContinue: startCreator }) }));
    if (phase === 'creating')
        return (_jsx(GameViewport, { children: _jsx(CharacterCreator, { initial: activeLoadout ?? {
                    ...defaultLoadout,
    ...
    return (_jsx(GameViewport, { children: _jsx(MainMenu, { playerName: playerName, accountLevel: accountLevel, loadout: activeLoadout ?? defaultLoadout, onCustomize: () => setPhase('creating'), heroLocked: heroLocked, view: mainView, onChangeView: setMainView, profile: profile, onSignOut: handleSignOut }) }));
}
