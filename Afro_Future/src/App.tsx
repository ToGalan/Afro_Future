...
function MissionScreen({ onExit, onOpenSkillTree, activeLoadout }: { onExit: () => void; onOpenSkillTree?: () => void; activeLoadout?: CharacterLoadout | null }){
  ...
  // Live XP from Firestore — replaces the placeholder XP_CUR/XP_MAX from useSkillsProgress
  const gameModeXP = usePlayerXP({ uid: profile?.uid ?? '', profile, saveProgress });
      ...
      <div className="absolute inset-0">
        <SoloMissionMap3D 
          ...
          heroVitals={{
            ...
            xp: { current: gameModeXP.xpInLevel, max: gameModeXP.xpForLevel },
            level: skillState.level,
            name: loadout.name,
        ...
        <div className="w-full max-w-[260px] aspect-[4/3] rounded-3xl border border-white/10 bg-[#12171f] overflow-hidden relative flex flex-col items-center justify-center">
          ...
          <div className="mt-2 flex flex-col items-center text-center">
            ...
            <div className="mt-1 text-[10px] text-gray-300 flex items-center gap-2">
              <span className="px-2 py-0.5 rounded-full bg-white/5 border border-white/10 tracking-wide">Lv {playerLevel}</span>
              <span>{loadout.archetype === 'MALE' ? 'Male' : 'Female'}</span>
            </div>
            {/* Hero XP progress bar */}
            <div className="mt-2 w-[80%] flex flex-col gap-0.5">
              ...
              <div className="h-1.5 rounded-full bg-white/10 overflow-hidden">
                <div
                  ...
                  style={{ width: `${xpForLevel > 0 ? Math.min(100, (xpInLevel / xpForLevel) * 100) : 0}%` }}
                />
              </div>
            </div>
          </div>
        ...
        <div className="w-full max-w-[260px] aspect-[4/3] rounded-3xl border border-white/10 bg-white/5 overflow-hidden relative flex flex-col items-center justify-center p-4">
          ...
          <div className="relative z-10 mt-3 flex flex-col items-center text-center">
            ...
            <div className="mt-2 w-[80%] flex flex-col gap-0.5">
              <div className="flex justify-between text-[9px] opacity-60">
                <span>Pet XP</span>
                <span>{petXp} / {xpToNext > 0 ? xpToNext : petXp}</span>
              </div>
              <div className="h-1.5 rounded-full bg-white/10 overflow-hidden">
                <div
