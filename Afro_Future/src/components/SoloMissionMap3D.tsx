...
// Types — canonical definitions live in src/types/map.ts
import type { TileType, ResourceType, Axial, TerrainChar, Tile, Actor, WorldPos } from '../types/map';
...
export default function SoloMissionMap3D({ 
  ...
  // ── Hero + pet state (position, spawn, patrol) — see src/hooks/useHeroState.ts + usePetState.ts ──
  const { hero, setHero, heroAvatarRef, recenterSignal, setRecenterSignal } = useHeroState({
    useChunks, chunkCenterAxial, tiles, spawnPos, profile, saveProgress,
  });
  const { pet, setPet } = usePetState({
    hero, tiles, useChunks, chunkCenterAxial, spawnPos, profile,
  });

  // ── Player XP system — gathering economy (1 collect = 1 XP) ────────────
  const playerXPSystem = usePlayerXP({
    uid: profile?.uid ?? '',
    profile,
    saveProgress,
  });
  ...
  const collectibles = useCollectibles({
    ...
    onHealHP,
    onRestoreEP,
    onPlayerXpGain: playerXPSystem.gainXP,
  });
  ...
  // (Hero/pet position, spawn, patrol and leash logic live in useHeroState + usePetState)
  ...
  // Seed from profile once per account (uid-keyed) — fixes race where profile loads
  // after the player has already moved, which made the old size===0 guard skip seeding.
  const exploredSeededUidRef = React.useRef<string | null>(null);
  useEffect(() => {
    if (profile?.uid && profile.progress?.explored && exploredSeededUidRef.current !== profile.uid) {
      exploredSeededUidRef.current = profile.uid;
      for (const k of profile.progress.explored) exploredRef.current.add(k);
      setExploredCount(exploredRef.current.size);
    }
  }, [profile?.uid, profile?.progress?.explored]);
  ...
  useEffect(() => {
    ...
    // Write directly to Firestore using a dot-path field update so explored saves never
    // compete with XP / collectible saves that share the saveProgress throttle.
    if (!profile?.uid) return;
    const uid = profile.uid;
    const to = setTimeout(() => {
      updateDoc(doc(db, 'players', uid), {
        'progress.explored': Array.from(exploredRef.current),
        updatedAt: serverTimestamp(),
      }).catch(() => {/* offline or rules error — silently ignore */});
    }, 800);
    return () => clearTimeout(to);
  }, [heroVisible, petVisible, explorationComplete, profile?.uid]);
  ...
  useEffect(() => {
    return () => {
      ...
      if (!uid || exploredRef.current.size === 0) return;
      updateDoc(doc(db, 'players', uid), {
        'progress.explored': Array.from(exploredRef.current),
        updatedAt: serverTimestamp(),
      }).catch(() => {});
  ...
  // ─── Avatar data — see src/hooks/useAvatarData.ts ───────────────────────────
  const { heroModelUrl, heroGender, heroParts, heroColors, avatarReady } = useAvatarData({ heroAvatar, profile });
  ...
  // Enemy spawn zones: near deserts or mountain passes
  // (enemy spawn / patrol hints removed pending gameplay implementation)

  function tileKey(t: Axial) { return `${t.q},${t.r}`; }
  // ── Keyboard: movement, abilities, items — see src/hooks/useMapInput.ts ─────
  const { collisionMessage, setCollisionMessage } = useMapInput({
    tilesByKey,
    axialBounds,
    saveProgress,
    updateHeroPosition,
    ...
    setAbilitySlots,
    handleItemUse,
    gainXPOnMove: petXPSystem.gainXPOnMove,
    setHero,
  });
  ...
  />
              ...
              <group position={[0, 0, 0]}>
                ...
                {culledTiles.map((t) => {
                  ...
                  const tileTop = heightFor(t);
                  // Skip terrain decorations on the hero's or pet's tile so the character is never occluded.
                  const isActorTile = (t.q === hero.pos.q && t.r === hero.pos.r) || (t.q === pet.pos.q && t.r === pet.pos.r);
                  return (
                    <group key={key} position={[x, 0, z]}>
                      <HexTile t={t} size={hexSize} onClick={() => {}} onHover={setHover} />
                      {/* Terrain decorations — skip on actor tiles so characters are always visible */}
                      {!isActorTile && (inVision || explored) && t.type === 'forest' && !collectibleMushrooms.has(key) && (
              ...
              <GameHUD
                ...
                hero={heroVitals ? {
                  ...
                  // Always use the internal playerXPSystem for live XP/level — it's the one
                  // wired to gainXP via useCollectibles. heroVitals.xp comes from a separate
                  // hook instance in App.tsx that never receives gainXP calls.
                  level: playerXPSystem.playerLevel,
                  hp: heroVitals.hp,
                  ep: heroVitals.ep,
                  xp: { current: playerXPSystem.xpInLevel, max: playerXPSystem.xpForLevel },
                ...
                pet={petData ? {
                  ...petData,
                  // Always use internal petXPSystem for live XP/level — it's wired to
                  // gainXPOnMove via useMapInput. petData.level is static from loadout.
                  level: petXPSystem.petLevel,
                  xp: { current: petXPSystem.petXp, max: petXPSystem.xpToNext },
                } : {
                  name: 'Pet',
                  level: petXPSystem.petLevel,
